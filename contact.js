...
function highlightInvalidField(field) {
  ...
  if (!errorMsg) {
    ...
    errorMsg.className =
      "error-message text-red-500 text-sm mt-1 ml-2 flex items-center";
    errorMsg.innerHTML = `<i class="fas fa-exclamation-circle mr-1"></i> ${getErrorMessage(
      field
    )}`;
...
function initContactAnimations() {
  ...
  if (!("IntersectionObserver" in window)) {
    ...
    const animatedElements = document.querySelectorAll(
      "#contact .glassmorphism, #contact .contact-info-item"
    );
  ...
  contactInfoItems.forEach((item, index) => {
    ...
    item.style.transition = `opacity 0.6s ease ${
      index * 0.1 + 0.2
    }s, transform 0.6s ease ${index * 0.1 + 0.2}s`;

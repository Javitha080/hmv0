...
function initGallery() {
  ...
  const filterButtons = document.querySelectorAll(
    "#gallery .flex.justify-center button"
  );
...
function openLightbox(imgSrc, title, description) {
  ...
  lightbox.className =
    "fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 opacity-0 transition-opacity duration-300";

  const lightboxContent = document.createElement("div");
  lightboxContent.className =
    "relative bg-white/10 backdrop-blur-md rounded-xl overflow-hidden max-w-4xl w-full transform scale-95 transition-transform duration-300";

  const closeBtn = document.createElement("button");
  closeBtn.className =
    "absolute top-4 right-4 w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center z-10 text-white hover:bg-white/30 transition-all duration-300";
...
function initParallaxEffect() {
  ...
  const bgElements = gallerySection.querySelectorAll(
...
function initNewsSection() {
  ...
  expandIcons.forEach((icon) => {
    icon.addEventListener("click", function (e) {
      ...
      const description =
        newsCard.querySelector("p:nth-of-type(2)").textContent;

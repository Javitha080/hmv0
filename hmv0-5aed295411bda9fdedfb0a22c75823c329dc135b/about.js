...
function initCounters() {
  ...
  counters.forEach((counter) => {
    ...
    highlight.className =
      "absolute inset-0 bg-primary/10 rounded-lg scale-110 opacity-0";
    ...
    particles.forEach((particle, index) => {
      ...
      if (scrollRelative > -500 && scrollRelative < 1000) {
        particle.style.transform = `translate(${
          direction * scrollRelative * speed
        }px, ${scrollRelative * speed * 0.5}px)`;
      }
    });
  });
}
...
function enhanceCardInteractions() {
  ...
  cards.forEach((card) => {
    ...
    lightReflection.className =
      "absolute inset-0 bg-gradient-to-tr from-white/0 via-white/10 to-white/0 opacity-0 transition-opacity duration-500 pointer-events-none";
    ...
    card.addEventListener("mousemove", function (e) {
      ...
      card.style.boxShadow = `
                0 ${15 + Math.abs(rotateY)}px ${
        25 + Math.abs(rotateX) * 2
      }px rgba(0, 0, 0, 0.1),
                ${rotateY * 0.5}px ${rotateX * -0.5}px ${
        10 + Math.abs(rotateY)
      }px rgba(0, 0, 0, 0.05)
            `;
      ...
      if (lightReflection) {
        ...
        lightReflection.style.background = `
                    radial-gradient(
                        circle at ${(mouseX / cardRect.width + 0.5) * 100}% ${
          (mouseY / cardRect.height + 0.5) * 100
        }%, 
      ...
      if (icon) {
        icon.style.transform = `translate(${mouseX * 0.08}px, ${
          mouseY * 0.08
        }px) scale(1.1)`;
        icon.style.textShadow = `${rotateY * 0.05}px ${
          rotateX * -0.05
        }px 5px rgba(0, 0, 0, 0.15)`;
      }

      if (title) {
        title.style.transform = `translate(${mouseX * 0.04}px, ${
          mouseY * 0.04
        }px)`;
      }

      if (cornerElement) {
        cornerElement.style.transform = `translate(${mouseX * -0.03}px, ${
          mouseY * -0.03
        }px)`;
      }

      if (statCounter) {
        statCounter.style.transform = `translate(${mouseX * 0.02}px, ${
          mouseY * 0.02
        }px)`;
      }

      if (arrow) {
        arrow.style.transform = `translate(${mouseX * 0.1}px, ${
          mouseY * 0.1
        }px)`;
      }
    });
...
function enhanceImageInteractions() {
  const historyImage = document.querySelector(
    "#about .md\\:w-1\\/2.relative.group.overflow-hidden img"
  );
  const imageOverlay = document.querySelector(
    "#about .absolute.inset-0.bg-gradient-to-r"
  );
  ...
  magnifyIcon.className =
    "absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-3xl opacity-0 transition-opacity duration-300 z-20";
...
function createImageModal(imageSrc) {
  ...
  closeBtn.className =
    "absolute top-4 right-4 text-white text-2xl bg-primary/80 w-10 h-10 rounded-full flex items-center justify-center";
...
function enhanceButtonAnimations() {
  const learnMoreBtn = document.querySelector(
    "#about .group.relative.overflow-hidden.mt-4"
  );
...
function initMouseParallax() {
  ...
  aboutSection.addEventListener("mousemove", function (e) {
    ...
    particles.forEach((particle, index) => {
      ...
      particle.style.transform = `translate(${
        xPercent * intensity * speedFactor
      }px, ${yPercent * intensity * speedFactor}px)`;
    });
  });
...
function improveAccessibility() {
  ...
  const cards = document.querySelectorAll("#about .group");
  const historyImage = document.querySelector(
    "#about .md\\:w-1\\/2.relative.group.overflow-hidden"
  );
  const learnMoreBtn = document.querySelector(
    "#about .group.relative.overflow-hidden.mt-4"
  );
  ...
  if (!document.querySelector(".skip-link")) {
    ...
    skipLink.className =
      "skip-link sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-primary text-white px-4 py-2 z-50 rounded";
    skipLink.href = "#about";

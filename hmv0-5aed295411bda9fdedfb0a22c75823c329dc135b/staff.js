...
function initStaffCards() {
  ...
  staffCards.forEach((card) => {
    ...
    card.addEventListener("mousemove", function (e) {
      ...
      const glare =
        this.querySelector(".card-glare") || document.createElement("div");
      if (!this.querySelector(".card-glare")) {
        ...
        glare.style.background =
          "radial-gradient(circle at var(--mouse-x) var(--mouse-y), rgba(255,255,255,0.15) 0%, rgba(255,255,255,0) 50%)";
    ...
    card.addEventListener("click", function () {
      ...
      const staffRole = this.querySelector(
        ".absolute.-bottom-4"
      ).textContent.trim();
      const staffImage = this.querySelector("img").src;
      const staffQualification =
        this.querySelector("p:nth-of-type(1)").textContent.trim();
      const staffDescription =
        this.querySelector("p:nth-of-type(2)").textContent.trim();
    ...
    card.setAttribute(
      "aria-label",
      `View profile of ${card.querySelector("h3").textContent}`
    );
...
function createStaffFilters() {
  ...
  filterContainer.className =
    "staff-filters flex flex-wrap justify-center gap-3 mb-10";
  ...
  filters.forEach((filter, index) => {
    ...
    button.className = `staff-filter-btn px-4 py-2 rounded-full flex items-center gap-2 transition-all duration-300 ${
      index === 0
...
function initStaffLightbox() {
  ...
  if (!document.getElementById("staff-lightbox")) {
    ...
    lightbox.className =
      "fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm opacity-0 pointer-events-none transition-opacity duration-300";
    ...
    document.addEventListener("keydown", function (e) {
      if (
        e.key === "Escape" &&
        !lightbox.classList.contains("pointer-events-none")
      ) {
        closeStaffLightbox();
      }
    });
